...
enum StudioAutoBind: String, CaseIterable, Identifiable {
    case none
    case timeSeconds
    case textureWidth
    case textureHeight
    ...
    static func infer(from parameterName: String) -> StudioAutoBind {
...
struct StudioChoiceOption: Identifiable, Equatable {
    var id: UUID = UUID()
    var key: String
    var label: String
}

struct StudioParameterDraft: Identifiable, Equatable {
    var id: UUID = UUID()
    var name: String
    var type: StudioParamType
    var defaultNumber: Double
    var minNumber: Double
    var maxNumber: Double
    var defaultBool: Bool
    var defaultChoiceKey: String = ""
    var choiceOptions: [StudioChoiceOption] = []
    var autoBind: StudioAutoBind

    var isAutoBound: Bool { autoBind != .none }

    static func `default`(named name: String = "param") -> StudioParameterDraft {
        StudioParameterDraft(
            name: name,
            type: .float,
            defaultNumber: 0,
            minNumber: 0,
...
private enum StudioScalarValueType {
    case float
    case int
    case uint
    case bool
...
final class EffectsStudioViewModel: ObservableObject {
    ...
    var editableParameterDefinitions: [StudioParameterDraft] {
    ...
    func refreshRuntimeEffectList() {
        ...
        let effects = EffectRegistry.availableEffectTypes
            ...
            .map { StudioRuntimeEffectChoice(type: $0.type, displayName: $0.displayName) }
    ...
    func chooseFileSource() {
        ...
        panel.title = "Choose Studio Source"
    ...
    func restoreInitialSource(from library: MediaLibrary, preferredLength: Double = 8.0) {
        ...
        guard let kindRaw = defaults.string(forKey: sourceKindDefaultsKey),
              let kind = PersistedSourceKind(rawValue: kindRaw) else {
        ...
        switch kind {
        case .file:
            guard let path = defaults.string(forKey: sourceValueDefaultsKey), !path.isEmpty else {
                loadRandomSource(from: library, preferredLength: preferredLength)
                return
            }
        ...
        case .photos:
            guard let identifier = defaults.string(forKey: sourceValueDefaultsKey), !identifier.isEmpty else {
                loadRandomSource(from: library, preferredLength: preferredLength)
                return
            }
    ...
    private static func sanitizedChoiceOptions(for param: StudioParameterDraft) -> [StudioChoiceOption] {
        var seenKeys = Set<String>()
        var sanitized: [StudioChoiceOption] = []
    ...
    private static func resolvedChoiceDefaultKey(
        for param: StudioParameterDraft,
        options: [StudioChoiceOption]? = nil
    ) -> String {
    ...
    private func runtimeParameterEntry(from param: StudioParameterDraft) -> RuntimeMetalParameterSchemaEntry? {
        switch param.type {
        case .float:
    ...
    private static func defaultValue(for param: StudioParameterDraft) -> AnyCodableValue {
        switch param.type {
        case .float:
    ...
    private static func parameterSpec(for param: StudioParameterDraft) -> ParameterSpec {
        switch param.type {
        case .float:
        ...
        for name in order {
            ...
            case .choice:
                let options = (entry.options ?? []).map {
                    StudioChoiceOption(key: $0.key, label: $0.label)
                }
    ...
    private static func paramType(from raw: String) -> StudioParamType? {
    ...
    private static func scalarType(for dataType: MTLDataType) -> StudioScalarValueType? {
        switch dataType {
        case .float:
            return .float
        case .int:
    ...
    private static func scalarSize(for type: StudioScalarValueType) -> Int? {
        switch type {
        case .float, .int, .uint:
            return 4
        case .bool:
    ...
    private func persistLastFileSource(url: URL) {
        let defaults = UserDefaults.standard
        defaults.set(PersistedSourceKind.file.rawValue, forKey: sourceKindDefaultsKey)
        defaults.set(url.path, forKey: sourceValueDefaultsKey)
    }

    private func persistLastPhotosSource(identifier: String) {
        let defaults = UserDefaults.standard
        defaults.set(PersistedSourceKind.photos.rawValue, forKey: sourceKindDefaultsKey)
        defaults.set(identifier, forKey: sourceValueDefaultsKey)
    }

    private func persistLastSourceSample() {
        let defaults = UserDefaults.standard
        defaults.set(PersistedSourceKind.sample.rawValue, forKey: sourceKindDefaultsKey)
        defaults.removeObject(forKey: sourceValueDefaultsKey)
    }
    ...
    private static let defaultParameters: [StudioParameterDraft] = [
        StudioParameterDraft(
            name: "offsetAmount",
            type: .float,
            defaultNumber: 10,
    ...
    private static let systemParameterBlueprints: [StudioParameterDraft] = [
        StudioParameterDraft(
            name: "timeSeconds",
            type: .float,
            defaultNumber: 0,
...
private struct StudioParameterDefinitionRow: View {
    @Binding var parameter: StudioParameterDraft
    let onChanged: () -> Void
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 8) {
                ...
                Picker("Type", selection: Binding(
                        ...
                        if parameter.type == .choice {
                            if parameter.choiceOptions.isEmpty {
                                parameter.choiceOptions = [
                                    StudioChoiceOption(key: "option1", label: "Option 1")
                                ]
                            }
                    ...
                    ForEach(StudioParamType.allCases) { kind in
            ...
            HStack(spacing: 8) {
                Picker("Binding", selection: Binding(
                    ...
                    ForEach(StudioAutoBind.allCases) { mode in
...
struct EffectsStudioView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            if !didLoadStudioUIState {
                applyStudioUIState(studioUIStore.value)
                didLoadStudioUIState = true
            }
            tabMonitor.start(
                shouldHandleEvent: shouldHandleStudioTab(event:),
                onTabPressed: toggleStudioCleanScreen
            )
        ...
        .onChange(of: showLogOverlay) { _, _ in persistStudioUIState() }
    ...
    private func bindingForParameter(id: UUID) -> Binding<StudioParameterDraft>? {
    ...
    private func applyStudioUIState(_ state: EffectsStudioUIState) {
        ...
        isApplyingStoredStudioUIState = false
    }

    private func persistStudioUIState() {
    ...
    private func zIndex(for panel: StudioPanelID) -> Double {
        guard let index = panelStack.firstIndex(of: panel) else { return 1 }
        return Double(index + 1)
    }

    private func bringPanelToFront(_ panel: StudioPanelID) {
    ...
    private func shouldHandleStudioTab(event: NSEvent) -> Bool {
        ...
        guard isStudioWindow(NSApp.keyWindow) else { return false }
        if let eventWindow = event.window, !isStudioWindow(eventWindow) {
            return false
        }
        return true
    }
    ...
    private func toggleStudioCleanScreen() {
        if let snapshot = cleanScreenSnapshot {
            ...
            showStudioChrome = snapshot.showChrome

            cleanScreenSnapshot = nil
            focusStudioHostWindow()
            return
        }
...
@MainActor
private final class StudioTabKeyMonitor: ObservableObject {
...
@MainActor
private final class StudioPanelWindowController: ObservableObject {
...
private struct StudioPanelHostBridge: NSViewRepresentable {
    ...
    final class Coordinator {
        var controller: StudioPanelWindowController
        init(controller: StudioPanelWindowController) {
...
private struct FloatingStudioPanel<Content: View>: View {
    let title: String

//
//  DreamAudioController.swift
//  Hypnograph
//
//  Manages audio output devices and volumes for preview and live players.
...
/// Manages audio output routing for Dream's preview and live players
@MainActor
final class DreamAudioController: ObservableObject {

    // MARK: - Published State

    /// Selected audio output device for Preview player (nil = system default)
    @Published var previewAudioDevice: AudioOutputDevice?
    ...
    init(settingsStore: SettingsStore, livePlayer: LivePlayer) {
    ...
    private func setupSubscriptions() {
        ...
        $liveAudioDevice
            ...
            .sink { [weak self] device in
                ...
                print("🔊 DreamAudioController: liveAudioDevice changed to \(device?.name ?? "nil"), uid=\(deviceUID ?? "System Default")")
                self.livePlayer?.setAudioDevice(deviceUID)
                self.saveAudioSettings()
            }
            .store(in: &subscriptions)

        // Preview audio device changes (for saving settings)
        $previewAudioDevice
            .receive(on: RunLoop.main)
            .dropFirst()
            .sink { [weak self] _ in
        ...
        // Preview volume changes (for saving settings)
        $previewVolume
            .receive(on: RunLoop.main)
            .dropFirst()
            .sink { [weak self] _ in
    ...
    private func loadAudioSettings(from settings: Settings) {
        // Load volumes
        ...
        print("🔊 DreamAudioController: Loaded audio settings - preview: \(previewAudioDevice?.name ?? "System Default") @ \(previewVolume), live: \(liveAudioDevice?.name ?? "System Default") @ \(liveVolume)")
    }
    ...
    private func handleDeviceListChange(_ devices: [AudioOutputDevice]) {
        ...
        if let preview = previewAudioDevice,
           ...
           !devices.contains(where: { $0.uid == preview.uid }) {
            print("🔊 DreamAudioController: Preview audio device '\(preview.name)' disconnected, switching to System Default")
        ...
        if let live = liveAudioDevice,
           ...
           !devices.contains(where: { $0.uid == live.uid }) {
            print("🔊 DreamAudioController: Live audio device '\(live.name)' disconnected, switching to System Default")

...
@MainActor
final class EffectsSession: ObservableObject {

    // MARK: - Published State

    /// The effect chains in this session (single source of truth)
    @Published private(set) var chains: [EffectChain] = [] {
        didSet {
    ...
    nonisolated var chainsSnapshot: [EffectChain] {
        _chainsLock.lock()
    ...
    var onChainUpdated: ((Int, EffectChain) -> Void)?
    
    /// Fired when the session is reloaded from disk
    var onSessionReloaded: (() -> Void)?
    ...
    convenience init(filename: String) {
        let directory = Environment.appSupportDirectory
            .appendingPathComponent("effect-libraries", isDirectory: true)
    ...
    func replaceChains(_ newChains: [EffectChain]) {
        chains = newChains
        markDirty()
        scheduleSave()
        onSessionReloaded?()
    ...
    func updateParameter(chainIndex: Int, effectIndex: Int?, key: String, value: AnyCodableValue) {
    ...
    func addEffectToChain(chainIndex: Int, effectType: String) {
        guard chainIndex >= 0 && chainIndex < chains.count else { return }
    ...
    func removeEffectFromChain(chainIndex: Int, effectIndex: Int) {
        guard chainIndex >= 0 && chainIndex < chains.count else { return }
    ...
    func reorderEffectsInChain(chainIndex: Int, fromIndex: Int, toIndex: Int) {
        guard chainIndex >= 0 && chainIndex < chains.count else { return }
    ...
    func setEffectEnabled(chainIndex: Int, effectIndex: Int, enabled: Bool) {
    ...
    func resetEffectToDefaults(chainIndex: Int, effectIndex: Int) {
        guard chainIndex >= 0 && chainIndex < chains.count else { return }
    ...
    func updateChainName(chainIndex: Int, name: String) {
        guard chainIndex >= 0 && chainIndex < chains.count else { return }
    ...
    /// Get chain by name
    func chain(named name: String) -> EffectChain? {
    ...
    func merge(chains newChains: [EffectChain]) {
        var merged = chains
    ...
    nonisolated static func loadBundledDefaults() -> [EffectChain] {

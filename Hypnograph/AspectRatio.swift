...
struct AspectRatio: Codable, Equatable, CustomStringConvertible, Hashable {
    ...
    /// Whether this is the "fill screen" sentinel
    var isFillScreen: Bool { displayString == "fill" }

    // MARK: - Common Presets

    /// Fill screen - uses the view's native aspect ratio (no letterboxing)
    static let fillScreen = AspectRatio(value: 0, displayString: "fill")
    ...
    static let menuPresets: [AspectRatio] = [
        .fillScreen, .ratio16x9, .ratio9x16, .ratio4x3, .ratio1x1
    ]
    ...
    /// Get the effective aspect ratio, resolving fillScreen to actual screen ratio
    func effectiveValue(for containerSize: CGSize) -> CGFloat {
        if isFillScreen {
    ...
    static func screenAspectRatio() -> CGFloat {
        #if canImport(AppKit)

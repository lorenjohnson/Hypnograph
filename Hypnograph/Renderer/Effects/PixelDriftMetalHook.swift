...
final class PixelDriftMetalHook: RenderHook {
    ...
    init(driftStrength: Float = 8.0, threshold: Float = 0.05, decay: Float = 0.6) {
    ...
    private func loadShader() {
        ...
        do {
            ...
            guard let function = library.makeFunction(name: "pixelDriftKernel") else {
                print("⚠️ PixelDriftMetalHook: Kernel not found")
                return
            }
            pipelineState = try device.makeComputePipelineState(function: function)
        } catch {
            print("⚠️ PixelDriftMetalHook: Pipeline error: \(error)")
        }
    }

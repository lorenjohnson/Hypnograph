//
//  UnifiedRenderer.swift
//  Hypnograph
//
//  Unified renderer using RenderEngine for both montage and sequence
//
...
/// Unified renderer that uses RenderEngine for export
final class UnifiedRenderer: HypnogramRenderer {
    ...
    private func render(recipe: HypnogramRecipe) async -> Result<URL, Error> {
        print("🎬 UnifiedRenderer: Starting render...")
        ...
        switch result {
        case .success(let url):
            print("✅ UnifiedRenderer: Export complete - \(url.lastPathComponent)")
            return .success(url)
        case .failure(let error):
            print("🔴 UnifiedRenderer: Export failed - \(error)")

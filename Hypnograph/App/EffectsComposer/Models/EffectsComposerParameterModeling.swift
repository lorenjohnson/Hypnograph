...
enum EffectsComposerParameterModeling {
    ...
    static func canonicalChoiceValue(
        for draft: EffectsComposerParameterDraft,
        rawValue: AnyCodableValue
    ) -> AnyCodableValue {
        let options = sanitizedChoiceOptions(for: draft)
    ...
    static func runtimeParameterEntry(from param: EffectsComposerParameterDraft) -> RuntimeMetalParameterSchemaEntry? {
        switch param.type {
        case .float:
    ...
    static func parameterDrafts(from manifest: RuntimeMetalEffectManifest) -> [EffectsComposerParameterDraft] {
        ...
        var result: [EffectsComposerParameterDraft] = []
        for name in order {
            guard let entry = manifest.parameters[name] else { continue }
            guard let type = paramType(from: entry.type) else { continue }

            let autoBind: EffectsComposerAutoBind = autoBoundSet.contains(name) ? EffectsComposerAutoBind.infer(from: name) : .none
            switch type {
            case .bool:
                result.append(
            ...
            case .choice:
                let options = (entry.options ?? []).map {
                    EffectsComposerChoiceOption(key: $0.key, label: $0.label)
                }
    ...
    static let defaultParameters: [EffectsComposerParameterDraft] = [
        EffectsComposerParameterDraft(
            name: "offsetAmount",
            type: .float,
            defaultNumber: 10,
    ...
    static let systemParameterBlueprints: [EffectsComposerParameterDraft] = [
        EffectsComposerParameterDraft(
            name: "timeSeconds",
            type: .float,
            defaultNumber: 0,

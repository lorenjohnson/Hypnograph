...
//  Hypnograph
//
//  Main feature: video/image composition with a single preview path.
//
...
final class Main: ObservableObject {
    ...
    let audioController: MainAudioController
    ...
    init(state: HypnographState, renderQueue: RenderEngine.ExportQueue) {
        ...
        self.player = MainPlayerState(config: state.settings.playerConfig, effectsSession: effectsLibrarySession)
        ...
        self.audioController = MainAudioController(settingsStore: state.settingsStore, livePlayer: livePlayer)

        // Forward player state changes to Main's objectWillChange for SwiftUI reactivity
    ...
    private func addSourceToPlayer(_ player: MainPlayerState, length: Double? = nil) {
    ...
    private func addSourceToPlayer(_ player: MainPlayerState, mediaClip: MediaClip) {

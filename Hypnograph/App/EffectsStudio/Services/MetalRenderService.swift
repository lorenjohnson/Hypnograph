...
struct MetalRenderService {
    ...
    func compile(
        device: MTLDevice,
        sourceCode: String,
        parameters: [EffectsStudioParameterDraft],
    ...
    private func generatedParamStructSource(parameters: [EffectsStudioParameterDraft]) -> String {
        var lines: [String] = [
            "struct HypnoParams {"
        ]
    ...
    private func buildParameterBufferLayout(
        ...
        activeBindings: RuntimeMetalBindingsManifest
    ) -> EffectsStudioParamBufferLayout? {
        ...
        if let structType = bufferArg.bufferStructType {
            members = structType.members
                ...
                .compactMap { member in
                    guard let valueType = EffectsStudioParameterModeling.scalarType(for: member.dataType),
                          let size = EffectsStudioParameterModeling.scalarSize(for: valueType) else {
                        return nil
                    }

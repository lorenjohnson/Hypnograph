(() => {
  ...
  if (!breadcrumbEl || !documentEl || !docsSidebarEl || !docsTreeEl || !docsSidebarToggleEl || !topbarNavEl) {
    return;
  }
  ...
  function createTopbarSeparator() {
    const separator = document.createElement("span");
    separator.className = "topbar-nav-sep";
    separator.textContent = "|";
    return separator;
  }
  ...
  function createGithubLink() {
    const link = document.createElement("a");
    link.href = "https://github.com/lorenjohnson/Hypnograph";
    link.target = "_blank";
    link.rel = "noopener noreferrer";
    link.className = "topbar-nav-github";
    link.setAttribute("aria-label", "View Hypnograph on GitHub");
    link.innerHTML = `
      <svg viewBox="0 0 16 16" aria-hidden="true" focusable="false">
        ...
        -.36-1.02.08-2.12 0 0 .67-.21 2.2.82a7.54 7.54 0 0 1 2-.27c.68 0 1.36.09 2
      ...
      <span>Hypnograph on GitHub</span>
    `;
    return link;
  }
  ...
  function renderTopbarNav() {
    ...
    if (isHomepage) {
      topbarNavEl.appendChild(createNavLink("Docs", "/docs"));
    } else {
      topbarNavEl.appendChild(createNavLink("Home", "/"));
    }

    topbarNavEl.appendChild(createTopbarSeparator());
    topbarNavEl.appendChild(createGithubLink());
  }

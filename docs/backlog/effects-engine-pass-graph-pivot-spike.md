...
So this spike should decide whether pass-graph is the real destination or just an interesting side path. If the answer is yes, it should also clarify what that means for the current runtime, for Effects Studio, and for how much compatibility work matters. The current working bias is that a pass-graph-first engine is probably the right long-term direction, and that OFX is at most an adapter or interoperability consideration rather than the core model. But that bias should be tested, not assumed.
...
- MUST say what this means for Effects Studio in the transition period.
...
If pass-graph still looks right after that comparison, the spike should end with a concise go/no-go decision, a transition stance for the current runtime, a short note on how Effects Studio fits into that transition, and an active-ready implementation brief. If it does not look right, the spike should say why and make the case for continuing with the current linear system more intentionally rather than leaving the question half-open.

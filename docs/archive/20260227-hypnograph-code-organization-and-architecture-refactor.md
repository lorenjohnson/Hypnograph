...
Clarify and simplify app architecture after the runtime-effects + Effects Composer spike, without changing product behavior.
...
- Two primary windows now exist:
  ...
  - Studio window (`Window("Effects Composer", id: "effectsStudio")`) via `EffectsComposerView`.
...
- App code is now rooted under `Hypnograph/App` with explicit domains:
  ...
  - `App/Studio/` (main runtime + views + persistence)
  - `App/EffectsComposer/` (studio runtime + views + persistence)
  - `App/Common/` (shared support/utilities/components)
- `SessionStore`, `HypnogramStore`, and main settings persistence live under `App/Studio/Persistence`.
- Studio UI/source persistence is now owned by `EffectsComposerSettingsStore` under `App/EffectsComposer/Persistence`.
- `EffectsComposerView.swift` remains large and is targeted for Phase 3 splitting.
...
Hypnograph/
  App/
    ...
    EffectsComposer/
      ...
      EffectsComposerPanelWindowController.swift
      Persistence/
...
- Keep `Main` and `EffectsComposer` relatively flat at first; add deeper subfolders only when needed.
...
- Settings ownership is explicit:
  ...
  - `EffectsComposerSettingsStore` lives in `EffectsComposer/Persistence`.
...
- Keep `EffectsComposer` explicit and separate.
...
2. Execute Phase 2 settings split (`AppSettings`, `MainSettings`, `EffectsComposerSettings`).
...
- Lock settings ownership model (`AppSettings`, `MainSettings`, `EffectsComposerSettings`).
 - Status: complete.

## Phase 1: Structural Move + Main Rename

- Create `App/` with nested `Main`, `EffectsComposer`, and `Common` folders.
...
- Split settings into three stores:
  ...
  - `EffectsComposerSettingsStore` for studio editor/panel behavior.
- Move `SessionStore` under `Main/Persistence` unless Effects Composer gains real usage.
- Keep `Common` free of domain settings ownership.
 - Status: complete (`AppSettingsStore`, `MainSettingsStore`, `EffectsComposerSettingsStore` wired).
...
2. Studio settings policy:
   - Selected: `AppSettings` + `MainSettings` + `EffectsComposerSettings`.
...
Proceed with Phase 3: targeted audit/split of oversized files (starting with `EffectsComposerView` and optionally `HypnographAppDelegate`) plus cleanup of deferred naming/session terminology.

## Completion Summary

- Phase 0 completed (scope and naming/settings policy alignment).
- Phase 1 completed (App/Studio/EffectsComposer/Common structural reorganization and Dream->Main transition).
- Phase 2 completed (settings split to AppSettings/MainSettings/EffectsComposerSettings).
- Phase 3 completed via follow-on project: `archive/20260227-hypnograph-code-audit-and-convergence.md`.
- Next follow-on project (completed): `archive/20260227-fx-studio-architecture-refactor.md`.
